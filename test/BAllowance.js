...
contract('BAllowance', function (accounts) {
	...
	beforeEach(async () => {
		token = await BAllowance.new("Stacking Salmon", "STACKING SALMON");
	});
	...
	it('checkBorrowAllowance fail', async () => {
		...
		await expectRevert(token.checkBorrowAllowance(user, other, TEST_AMOUNT), "Stacking Salmon: BORROW_NOT_ALLOWED");
		await token.borrowApprove(other, TEST_AMOUNT, {from: user});
		await expectRevert(token.checkBorrowAllowance(user, other, TEST_AMOUNT.add(new BN(1))), "Stacking Salmon: BORROW_NOT_ALLOWED");
