...
contract('Highlevel', function (accounts) {
	...
	it('deploy lending pool', async () => {
		...
		await factory.obj.stackingSalmonPriceOracle.setPrice(uniswapV2Pair.address, uq112(price0A / price1A));
	...
	it('liquidation fail', async () => {
		await factory.obj.stackingSalmonPriceOracle.setPrice(uniswapV2Pair.address, uq112(price0B / price1B));
		...
		await expectRevert(borrowable0.liquidate(borrower, liquidator), 'Stacking Salmon: INSUFFICIENT_SHORTFALL');
	});
	
	it('liquidate token0', async () => {
		await factory.obj.stackingSalmonPriceOracle.setPrice(uniswapV2Pair.address, uq112(price0C / price1C));
		const {liquidity, shortfall} = await collateral.accountLiquidity.call(borrower);
