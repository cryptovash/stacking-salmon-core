...
contract('Collateral', function (accounts) {
	...
	describe('getPrices', () => {
		...
		before(async () => {
			...
			await collateral.setPriceOracle(factory.obj.stackingSalmonPriceOracle.address);
		...
		[
			...
			it(`getPrices for ${JSON.stringify(testCase)}`, async () => {
				...
				await factory.obj.stackingSalmonPriceOracle.setPrice(underlying.address, uq112(priceOracle));
		...
		it(`fail if price(0|1) <= 100`, async () => {
			...
			await factory.obj.stackingSalmonPriceOracle.setPrice(underlying.address, priceUQ112);
			await underlying.setTotalSupply(totalSupply);
			await underlying.setReserves(reserve0, reserve1);
			await expectRevert(collateral.getPrices(), 'Stacking Salmon: PRICE_CALCULATION_ERROR');
	...
	[
		...
		describe(`Collateral tests for ${JSON.stringify(testCase)}`, () => {
			...
			it(`transfer:fail`, async () => {
				...
				await expectRevert(collateral.transfer(address(0), transferAmount, {from: user}), 'Stacking Salmon: INSUFFICIENT_LIQUIDITY');
			});
	...
	describe('seize', () => {
		...
		it(`fail if msg.sender is not borrowable`, async () => {
			await expectRevert(collateral.seize(address(0), address(0), '0'), "Stacking Salmon: UNAUTHORIZED");
		});
	...
	describe('flash redeem', () => {
		...
		before(async () => {
			...
			callee = (await StackingSalmonCallee.new(recipient.address, collateral.address)).address;
		});
		...
		it('redeem fails if redeemTokens is not enough', async () => {
			...
			await expectRevert(collateral.flashRedeem(user, redeemAmount, '0x'), 'Stacking Salmon: INSUFFICIENT_REDEEM_TOKENS');
		});
		...
		it('redeem fails if redeemAmount exceeds cash', async () => {
			await expectRevert(
				...
				'Stacking Salmon: INSUFFICIENT_CASH'
			);
		});
	...
	describe('reentrancy', () => {
		...
		it(`borrow reentrancy`, async () => {
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [1])), 'Stacking Salmon: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [2])), 'Stacking Salmon: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [3])), 'Stacking Salmon: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [4])), 'Stacking Salmon: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [5])), 'Stacking Salmon: REENTERED');
