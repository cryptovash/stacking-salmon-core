...
const StackingSalmonERC20 = artifacts.require('StackingSalmonERC20Harness');
...
const StackingSalmonCallee = artifacts.require('StackingSalmonCallee');
...
async function makeErc20Token(opts = {}) {
	...
	return await StackingSalmonERC20.new(name, symbol);
}
...
async function makeStackingSalmonPriceOracle(opts = {}) {
	return await MockOracle.new();
}

//STACKING SALMON DEPLOYER
...
async function makeFactory(opts = {}) {
	...
	const uniswapV2Factory = opts.uniswapV2Factory || await makeUniswapV2Factory(opts);
	const stackingSalmonPriceOracle = opts.stackingSalmonPriceOracle || await makeStackingSalmonPriceOracle(opts);
	const factory = await Factory.new(admin, reservesAdmin, bDeployer.address, cDeployer.address, stackingSalmonPriceOracle.address);
	return Object.assign(factory, {obj: {admin, reservesAdmin, bDeployer, cDeployer, uniswapV2Factory, stackingSalmonPriceOracle,
