...
contract('StackingSalmonERC20', function (accounts) {
	let root = accounts[0];
	let user = accounts[1];
	...
	beforeEach(async () => {
		token = await StackingSalmonERC20.new(NAME, SYMBOL);
	...
	it('transfer:fail', async () => {
		await expectRevert(token.transfer(other, TOTAL_SUPPLY.add(new BN(1)), {from: user}), 'Stacking Salmon: TRANSFER_TOO_HIGH');
		await expectRevert(token.transfer(user, "1", {from: other}), 'Stacking Salmon: TRANSFER_TOO_HIGH');
	});
	...
	it('transferFrom:fail', async () => {
		await expectRevert(token.transferFrom(user, other, TEST_AMOUNT, {from: other}), 'Stacking Salmon: TRANSFER_NOT_ALLOWED');
	});
