...
contract('BSetter', function (accounts) {
	...
	it('permissions check', async () => {
		...
		await expectRevert(borrowable._setReserveFactor(RESERVE_FACTOR_TEST, {from: user}), 'Stacking Salmon: UNAUTHORIZED');
		await expectRevert(borrowable._setKinkUtilizationRate(KINK_UR_TEST, {from: user}), 'Stacking Salmon: UNAUTHORIZED');
		await expectRevert(borrowable._setAdjustSpeed(ADJUST_SPEED_TEST, {from: user}), 'Stacking Salmon: UNAUTHORIZED');
		await expectRevert(borrowable._setBorrowTracker(BORROW_TRACKER_TEST, {from: user}), 'Stacking Salmon: UNAUTHORIZED');
	});
	...
	it('reserve factory boundaries', async () => {
		...
		await expectRevert(borrowable._setReserveFactor(failMax, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
	});
	...
	it('kink utilization rate boundaries', async () => {
		...
		await expectRevert(borrowable._setKinkUtilizationRate(failMin, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
		...
		await expectRevert(borrowable._setKinkUtilizationRate(failMax, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
	});
	...
	it('adjust speed boundaries', async () => {
		...
		await expectRevert(borrowable._setAdjustSpeed(failMin, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
		...
		await expectRevert(borrowable._setAdjustSpeed(failMax, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
	});
});