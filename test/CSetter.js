...
contract('CSetter', function (accounts) {
	...
	it('permissions check', async () => {
		...
		await expectRevert(collateral._setSafetyMarginSqrt(SAFETY_MARGIN_TEST, {from: user}), 'Stacking Salmon: UNAUTHORIZED');
		await expectRevert(collateral._setLiquidationIncentive(LIQUIDATION_INCENTIVE_TEST, {from: user}), 'Stacking Salmon: UNAUTHORIZED');
	});
	...
	it('safety margin boundaries', async () => {
		...
		await expectRevert(collateral._setSafetyMarginSqrt(failMin, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
		...
		await expectRevert(collateral._setSafetyMarginSqrt(failMax, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
	});
	...
	it('liquidation incentive boundaries', async () => {
		...
		await expectRevert(collateral._setLiquidationIncentive(failMin, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
		...
		await expectRevert(collateral._setLiquidationIncentive(failMax, {from: admin}), 'Stacking Salmon: INVALID_SETTING');
	});
});