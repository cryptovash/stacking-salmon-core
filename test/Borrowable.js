...
contract('Borrowable', function (accounts) {
	...
	describe('borrow and repay', () => {
		...
		async function makeBorrow(params) {
			...
			if (maxBorrowable.lt(expectedAccountBorrows)) {
				await expectRevert(borrowAction, 'Stacking Salmon: INSUFFICIENT_LIQUIDITY');
				return false;
			}
		...
		before(async () => {
			...
			receiver = (await StackingSalmonCallee.new(recipient.address, underlying.address)).address;
		...
		it(`fail if cash is insufficient`, async () => {
			...
			await borrowable.sync();
			await expectRevert(borrowable.borrow(borrower, receiver, '1', '0x'), 'Stacking Salmon: INSUFFICIENT_CASH');			
		});

		it(`fail if not allowed`, async () => {
			await underlying.setBalanceHarness(borrowable.address, '1');
			await borrowable.sync();
			await expectRevert(borrowable.borrow(borrower, receiver, '1', '0x'), 'Stacking Salmon: BORROW_NOT_ALLOWED');			
		});
	...
	describe('liquidate', () => {
		...
		it(`fail if shortfall is insufficient`, async () => {
			...
			await expectRevert(borrowable.liquidate(borrower, liquidator), "Stacking Salmon: INSUFFICIENT_SHORTFALL");		
		});
		...
		it(`fail if there is not enough collateral`, async () => {
			...
			await expectRevert(borrowable.liquidate(borrower, liquidator), "Stacking Salmon: LIQUIDATING_TOO_MUCH");		
		});
	...
	describe('reentrancy', () => {
		...
		it(`borrow reentrancy`, async () => {
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [1])), 'Stacking Salmon: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [2])), 'Stacking Salmon: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [3])), 'Stacking Salmon: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [4])), 'Stacking Salmon: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [5])), 'Stacking Salmon: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [6])), 'Stacking Salmon: REENTERED');
