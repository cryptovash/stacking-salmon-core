const {
	...
	makeUniswapV2Pair,
	makeStackingSalmonPriceOracle,
	makeBDeployer,
	makeCDeployer,
	makeFactory
} = require('./Utils/StackingSalmon');
const {
	expectEqual,
...
contract('Factory', function (accounts) {
	...
	describe('constructor', () => {
		it("correctly initialize Factory", async () => {
			...
			const stackingSalmonPriceOracle = address(4);
			const factory = await Factory.new(admin, reservesAdmin, bDeployer, cDeployer, stackingSalmonPriceOracle);
			...
			expect(await factory.stackingSalmonPriceOracle()).to.eq(stackingSalmonPriceOracle);
		});		
	});
	...
	describe('create lending pool', () => {
		...
		it('revert if already exists', async () => {
			await expectRevert(factory.createCollateral(uniswapV2Pair1.address), "Stacking Salmon: ALREADY_EXISTS");
			await expectRevert(factory.createBorrowable0(uniswapV2Pair2.address), "Stacking Salmon: ALREADY_EXISTS");
			await expectRevert(factory.createBorrowable1(uniswapV2Pair3.address), "Stacking Salmon: ALREADY_EXISTS");			
		});
		...
		it('initialize revert if not all three contracts are deployed', async () => {
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair1.address), "Stacking Salmon: BORROWABLE1_NOT_CREATED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair2.address), "Stacking Salmon: COLLATERALIZABLE_NOT_CREATED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair3.address), "Stacking Salmon: BORROWABLE0_NOT_CREATED");
		}); 
		...
		it('factory can only be set once', async () => {
			const lendingPool = await factory.getLendingPool(uniswapV2Pair1.address);
			await expectRevert((await Collateral.at(lendingPool.collateral))._setFactory(), "Stacking Salmon: FACTORY_ALREADY_SET");
			await expectRevert((await Borrowable.at(lendingPool.borrowable0))._setFactory(), "Stacking Salmon: FACTORY_ALREADY_SET");
			await expectRevert((await Borrowable.at(lendingPool.borrowable1))._setFactory(), "Stacking Salmon: FACTORY_ALREADY_SET");
		}); 
		...
		it('stackingSalmonPriceOracle can be initialized or not', async () => {
			expect( (await factory.obj.stackingSalmonPriceOracle.getPair(uniswapV2Pair1.address)).initialized ).to.eq(false);
			expect( (await factory.obj.stackingSalmonPriceOracle.getPair(uniswapV2Pair2.address)).initialized ).to.eq(false);
			await factory.obj.stackingSalmonPriceOracle.initialize(uniswapV2Pair3.address);
			expect( (await factory.obj.stackingSalmonPriceOracle.getPair(uniswapV2Pair3.address)).initialized ).to.eq(true);
		});
		it('initialize', async () => {
		...
		it('stackingSalmonPriceOracle is initialized correctly', async () => {
			expect( (await factory.obj.stackingSalmonPriceOracle.getPair(uniswapV2Pair1.address)).initialized ).to.eq(true);
			expect( (await factory.obj.stackingSalmonPriceOracle.getPair(uniswapV2Pair2.address)).initialized ).to.eq(true);
			expect( (await factory.obj.stackingSalmonPriceOracle.getPair(uniswapV2Pair3.address)).initialized ).to.eq(true);
		});
		it('revert if already initialized', async () => {
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair1.address), "Stacking Salmon: ALREADY_INITIALIZED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair2.address), "Stacking Salmon: ALREADY_INITIALIZED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair3.address), "Stacking Salmon: ALREADY_INITIALIZED");
		});
	});
	...
	describe('admin', () => {
		...
		it("change admin", async () => {
			await expectRevert(factory._setPendingAdmin(root, {from: root}), "Stacking Salmon: UNAUTHORIZED");
			await expectRevert(factory._setPendingAdmin(root, {from: reservesAdmin}), "Stacking Salmon: UNAUTHORIZED");
			await expectRevert(factory._acceptAdmin({from: root}), "Stacking Salmon: UNAUTHORIZED");
		...
		it("change reserves admin", async () => {
			await expectRevert(factory._setReservesPendingAdmin(root, {from: root}), "Stacking Salmon: UNAUTHORIZED");
			await expectRevert(factory._setReservesPendingAdmin(root, {from: admin}), "Stacking Salmon: UNAUTHORIZED");
			await expectRevert(factory._acceptReservesAdmin({from: root}), "Stacking Salmon: UNAUTHORIZED");
		...
		it("change reserves manager", async () => {
			await expectRevert(factory._setReservesManager(reservesManager, {from: reservesManager}), "Stacking Salmon: UNAUTHORIZED");
			await expectRevert(factory._setReservesManager(reservesManager, {from: admin}), "Stacking Salmon: UNAUTHORIZED");
