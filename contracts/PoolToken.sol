...
contract PoolToken is IPoolToken, StackingSalmonERC20 {
    ...
    function _setFactory() external {
        require(factory == address(0), "StackingSalmon: FACTORY_ALREADY_SET");
        factory = msg.sender;
    }
    ...
    function mint(address minter) external nonReentrant update returns (uint256 mintTokens) {
        ...
        require(mintTokens > 0, "StackingSalmon: MINT_AMOUNT_ZERO");
    ...
    function redeem(address redeemer) external nonReentrant update returns (uint256 redeemAmount) {
        ...
        require(redeemAmount > 0, "StackingSalmon: REDEEM_AMOUNT_ZERO");
    ...
    function _safeTransfer(address to, uint256 amount) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "StackingSalmon: TRANSFER_FAILED");
    }

    // prevents a contract from calling itself, directly or indirectly.
    bool internal _notEntered = true;
    modifier nonReentrant() {
        require(_notEntered, "StackingSalmon: REENTERED");
        _notEntered = false;
        _;
        _notEntered = true;
    }
