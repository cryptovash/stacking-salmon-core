...
contract Borrowable is IBorrowable, PoolToken, BStorage, BSetter, BInterestRateModel, BAllowance {
    ...
    function borrow(
        ...
        require(borrowAmount <= _totalBalance, "StackingSalmon: INSUFFICIENT_CASH");
        _checkBorrowAllowance(borrower, msg.sender, borrowAmount);

        // optimistically transfer funds
        if (borrowAmount > 0) _safeTransfer(receiver, borrowAmount);
        if (data.length > 0) IStackingSalmonCallee(receiver).stackingSalmonBorrow(msg.sender, borrower, borrowAmount, data);
        ...
        if (adjustedBorrowAmount > repayAmount)
            require(ICollateral(collateral).canBorrow(borrower, address(this), accountBorrows), "StackingSalmon: INSUFFICIENT_LIQUIDITY");
